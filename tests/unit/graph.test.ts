...
describe("CardGraph", () => {
  ...
  describe("buildGraph", () => {
    ...
    it("should create tag-based edges when enabled", () => {
      ...
      expect(result.edges.filter((e) => e.type === EdgeType.TAG).length).toBe(
        2,
      );
  ...
  describe("graph analysis methods", () => {
    ...
    it("should find connected nodes", () => {
      ...
      expect(connected.map((n) => n.id)).toEqual(
  ...
  describe("graph options", () => {
    it("should respect includeBacklinks option", () => {
      ...
      const backlinkEdges = result.edges.filter(
        (e) => e.type === EdgeType.BACKLINK,
      );
