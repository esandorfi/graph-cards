...
describe("Choreographers Dataset Integration", () => {
  ...
  it("should create tag-based connections for dance styles", () => {
    ...
    const modernChoreographers = [
      "martha-graham",
      "merce-cunningham",
      "alvin-ailey",
    ];
    const modernConnections = tagEdges.filter(
      (e) =>
        modernChoreographers.includes(e.from) &&
        modernChoreographers.includes(e.to),
    );
  ...
  it("should find paths between related choreographers", () => {
    const graph = generator.generateFromDirectory("./dataset/choreographers");
    generator.graph.buildGraph(
      Array.from(graph.nodes.values()).map((n) => n.card),
    );

    // Path from Martha Graham to William Forsythe (through modern dance lineage)
    const path = generator.graph.getShortestPath(
      "martha-graham",
      "william-forsythe",
    );
