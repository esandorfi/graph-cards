...
describe("Cross-Dataset Integration", () => {
  ...
  it("should combine both datasets into single graph", () => {
    ...
    const choreographersCards = generator.parser.parseDirectory(
      "./dataset/choreographers",
    );
  ...
  it("should not create cross-dataset links unless explicitly referenced", () => {
    ...
    const crossDatasetEdges = combinedGraph.edges.filter(
      (edge) =>
  ...
  it("should handle tag overlaps between datasets", () => {
    ...
    const choreographerTags = new Set(
      choreographersCards.flatMap((c) => c.tags),
    );
    const emotionTags = new Set(emotionsCards.flatMap((c) => c.tags));
    const commonTags = [...choreographerTags].filter((tag) =>
      emotionTags.has(tag),
    );
  ...
  it("should find paths within datasets but not across unless connected", () => {
    ...
    const choreoPath = generator.graph.getShortestPath(
      "martha-graham",
      "merce-cunningham",
    );
  ...
  it("should handle different file counts gracefully", () => {
    ...
    const limitedChoreographers = generator.parser
      .parseDirectory("./dataset/choreographers")
      .slice(0, 3);
    const limitedEmotions = generator.parser
      .parseDirectory("./dataset/emotions")
      .slice(0, 5);

    const limitedGraph = generator.generateFromCards([
      ...limitedChoreographers,
      ...limitedEmotions,
    ]);
